...
- 只會貼上 final 結果，不會把 Google 仍在修正的中途結果寫入輸入框。
...
4. 當 Google 回傳 final 結果後，程式會自動把文字貼到目前輸入框。
...
中途文字不會被輸入到目前輸入框。原因是 Google Speech-to-Text 的 interim result 會不停修正，如果直接寫入輸入框，容易造成重複文字、刪除錯位或輸入法相容問題。Win Voice Input 只會在 final 結果出現後才貼上文字。
